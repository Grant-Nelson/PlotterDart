...
class Renderer extends IRenderer {
    ...
    void clear() {
        this._sout.clear();
        this._setDrawColor(this._backClr);
        var width = this._window.width();
        var height = this._window.height();
        this._sout.writeln("<svg viewBox=\"0 0 $width $height\" background-color: \""+this._drawClr+"\";>");
        this._setDrawColor(this._lineClr);
    }
    ...
    void drawPoint(double x, double y) {
        ...
        this._sout.writeln("  <circle cx=\"$x\" cy=\"$y\" r=\"$r\" fill=\""+this._drawClr+"\" />");
    }
    ...
    void drawLines(List<double> x1Coords, List<double> y1Coords,
        ...
        for (int i = x1Coords.length-1; i >= 0; --i) {
            this.drawLine(x1Coords[i], y1Coords[i],
                          x2Coords[i], y2Coords[i]);
        }
    }
    ...
    void drawRect(double x1, double y1, double x2, double y2) {
        ...
        this._sout.write("  <rect x=\"$x1\" y=\"$y2\" width=\"$width\" height=\"$height\" ");
        if (this._fillClr != null) {
            this._setDrawColor(this._fillClr);
            this._sout.write("fill=\""+this._drawClr+"\" ");
            this._setDrawColor(this._lineClr);
        } else {
    ...
    void drawStrip(List<double> xCoords, List<double> yCoords) {
        ...
        if (count >= 2) {
            ...
            this._sout.writeln("\" fill=\"none\" stroke=\""+this._drawClr+"\" />");
        }
